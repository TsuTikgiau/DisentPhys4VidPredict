...
class EvalMigHitWall(EvalMigBasis):

    def __init__(self, model, dataset, batch_size):
        super(EvalMigHitWall, self).__init__(model, dataset, batch_size)

        self.setting_len = self.inputs['setting'].shape[1].value

        self.properties_list = ['shape', 'scale', 'material', 'speed', 'position']
        self.num_value_list = [5, 9, 5, 13, 5]  # shape, scale, material, speed, position
        self.num_data = 1350
        self.test_frame = [3]
    ...
    def build_mask(self, setting, speed, position):
        ...
        # speed
        sub_mask = []
        for i in np.linspace(0.5, 12.5, self.num_value_list[3]):
            sub_mask.append(((speed - i < 0.50001) * (speed - i >= -0.5))[:, 0])
        mask.append(np.stack(sub_mask, 0))
        # position
        sub_mask = []
        for i in np.linspace(0.5, 4.5, self.num_value_list[4]):
            sub_mask.append(((position - i < 0.50001) * (position - i >= -0.5))[:, 0])
        mask.append(np.stack(sub_mask, 0))
        return mask
