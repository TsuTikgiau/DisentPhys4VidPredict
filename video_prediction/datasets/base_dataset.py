...
class BaseVideoDataset(object):
    def __init__(self, input_dir, mode='train', num_epochs=None, seed=None,
        ...
        if self.mode not in ('train', 'val', 'test', 'mig'):
    ...
    def make_batch(self, batch_size, reinit=False):
...
class VarLenFeatureVideoDataset(BaseVideoDataset):
    ...
    def parser(self, serialized_example):
        ...
        for example_name, (name, shape) in self.static_state.items():
            features[name] = tf.FixedLenFeature(shape, tf.int64)
        ...
        for example_name, (name, shape) in self.static_state.items():
            state_like_seqs[name] = features[name]
        return state_like_seqs, action_like_seqs
